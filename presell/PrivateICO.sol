...
contract PrivateICO is Ownable, ReentrancyGuard {
    ...
    function simulateClaim() external returns (uint256, uint256) {
        if (finalTokens[msg.sender] > 0)
            return (0, finalTokens[msg.sender]);

        if (commitments[msg.sender] == 0) return (0, 0);
